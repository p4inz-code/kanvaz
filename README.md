...
> **v3.5.3** — Board View, Connection System, and Inspector are stable and shipping.
...
npm run build:win
```
Output: `dist/Kanvaz Setup 3.5.3.exe` and `dist/Kanvaz 3.5.3.exe`
...
Boards are saved as `.kanvaz` files — plain JSON with version `"3.5.3"`. Media is embedded as base64 data URLs. Connections are stored as a top-level `connections` array alongside boards. Files from v2.x load cleanly with zero connections.

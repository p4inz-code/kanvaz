...
function registerIPC() {
  ...
  ipcMain.handle('model-load', function(event, filePath) {
    return fs.promises.stat(filePath).then(function(stats) {
      ...
      var ext = path.extname(filePath).toLowerCase().replace('.', '');
      var allowed = ['glb', 'gltf', 'obj', 'fbx', 'stl', 'ply', 'vox', 'usd', 'usda', 'usdc', 'usdz'];
      if (allowed.indexOf(ext) === -1) {
        return { ok: false, error: 'FILE_TYPE_INVALID', ext: ext };
      }
  ...
  /* ── v8.x Tier 3 — optional external DCC tool (Blender only, this round) ──
     .blend has no viable pure-JS/WASM parser (researched and rejected in
     the v7.x line, reconfirmed for v8.x — see docs/ROADMAP.md item 1) —
     so a locally-installed Blender is treated as an OPTIONAL EXTERNAL
     TOOL, never a bundled dependency: if found, shell out to its own
     headless batch mode to convert to .glb, then render the result
     through the existing, unmodified Three.js pipeline. If not found,
     the caller falls back to file-reference-only (same as any
     unsupported format).

     SECURITY, per the v8.x plan's own pre-go gap-check (non-negotiable,
     not a style preference): every argument below is passed as a real
     array element to child_process.spawn(), never concatenated into a
     shell string — spawn() with an argv array never invokes a shell, so
     there is no command-injection surface here even though blendPath
     is a user-supplied path. outputGlbPath is generated by THIS code,
     never derived from renderer/user input, closing the other half of
  ...
  /* Blender's own CLI convention: opening a .blend path as the first
     ...
     --python-expr script runs after the file is loaded; everything
     after a bare "--" is Blender's own signal to stop parsing its OWN
  ...
  ipcMain.handle('model-convert-external', function(event, filePath) {
    return fs.promises.stat(filePath).then(function(stats) {
      ...
      var ext = path.extname(filePath).toLowerCase().replace('.', '');
      /* Only .blend is actually wired to a real converter this round —
         .mb/.ma (Maya) and Houdini formats are explicitly future work
         on this same abstraction (see docs/ROADMAP.md item 1, Tier 3),
      ...
      return convertBlendToGlb(blenderExe, filePath, outputGlbPath).then(function() {
        return fs.promises.readFile(outputGlbPath).then(function(data) {
          return fs.promises.unlink(outputGlbPath).catch(function() {}).then(function() {
            return {
              ok: true,
              dataUrl: 'data:model/gltf-binary;base64,' + data.toString('base64'),
              modelFormat: 'glb',
              sizeMB: sizeMB,
              name: path.basename(filePath),
              originalPath: filePath,

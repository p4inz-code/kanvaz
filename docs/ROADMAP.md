...
2. **Build sequencing, decided 2026-09-15: features first, usability after, then a full testing phase, then ship — in that order, not interleaved.**
   - **Phase A — raw capability.** Build out every approved format tier and feature for this line before spending time on usability/UX polish. The user's own framing: "make sure v8 is full packed with tools and raw power," with "usability will be tackled after all features ship." This mirrors the v6.x arc's own precedent (its final UI-polish pass, v6.6.0, happened after all three pillars were functionally complete, not alongside them) — not a new pattern for this project.
   - **Phase B — usability/polish pass.** Only after Phase A is functionally complete. Scope not yet defined; revisit once Phase A's actual shape is known, the same way v6.6.0's scope was defined by what the finished pillars actually needed.
   - **Phase C — full testing, three parts, all before shipping anything:**
     1. **Live testing** — the CDP-driven manual-simulation technique already standard practice for this project (see the v7.x line's "no boot-test, actually click the button" lesson from v6.6.1/6.6.2).
     2. **Static testing** — `test/lint.js` + `test/validate.js` + whatever new test coverage this line's features need, same non-negotiable bar every prior release has met.
     3. **Person testing** — real working VFX/game dev/animation professionals (see "Plan for getting there," above) actually using it and giving feedback, before this becomes a numbered v8.x release.
   - **Only after all of Phase C is done does an actual v8.x version ship.** This sequencing is itself a decision recorded here, not just a description of normal process — it means no partial "v8.0.0 with half the formats" release is planned; the whole batch goes through this pipeline together.
